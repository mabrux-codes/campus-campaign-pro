// Country list with ISO-2 codes. Flag emoji is derived from code.
...
export function findCountry(nameOrCode: string | null | undefined): Country | undefined {
  if (!nameOrCode) return;
  const v = nameOrCode.toLowerCase();
  return COUNTRIES.find((c) => c.name.toLowerCase() === v || c.code.toLowerCase() === v);
}
